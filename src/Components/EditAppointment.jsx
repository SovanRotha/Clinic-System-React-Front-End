...
function EditAppointment() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const appointmentRes = await fetch(`http://localhost:8000/api/appointment/${id}`, {
        ...
        const [doctorRes, patientRes] = await Promise.all([
          fetch("http://localhost:8000/api/doctor", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://localhost:8000/api/patients", { headers: { Authorization: `Bearer ${token}` } }),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/appointment/${id}`, {
        method: "PUT",
