...
export default function ViewPatient() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      const patientResponse = await fetch(`http://localhost:8000/api/patients/${id}`, {
        headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
      });
      ...
      const [appointmentList, consultationList, prescriptionList, billList] = await Promise.all([
        fetchRecords(`http://localhost:8000/api/appointment?patient_id=${id}`, token),
        fetchRecords(`http://localhost:8000/api/consultation?patient_id=${id}`, token),
        fetchRecords(`http://localhost:8000/api/prescription?patient_id=${id}`, token),
        fetchRecords(`http://localhost:8000/api/bill?patient_id=${id}`, token),
      ]);
