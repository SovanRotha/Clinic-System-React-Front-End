...
export default function DashboardDoctor() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("http://localhost:8000/api/doctor/profile",       { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).catch(() => null),
      fetch("http://localhost:8000/api/AppointmentDoctor",     { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).catch(() => []),
      fetch("http://localhost:8000/api/patients",              { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).catch(() => []),
      fetch("http://localhost:8000/api/ConsultationDoctor",    { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).catch(() => []),
      fetch("http://localhost:8000/api/PrescriptionDoctor",    { headers: { Authorization: `Bearer ${token}` } }).then(r => r.json()).catch(() => []),
