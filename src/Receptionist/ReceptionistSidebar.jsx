...
import { LayoutDashboard, UserRound, CalendarClock, CreditCard, LogOut } from "lucide-react";
...
export default function ReceptionistSidebar() {
    ...
    const getActiveId = (pathname) => {
        const sorted = [...NAV_ITEMS].sort((a, b) => {
            const aLen = Math.max(...a.matchPaths.map((p) => p.length));
            const bLen = Math.max(...b.matchPaths.map((p) => p.length));
            return bLen - aLen;
        });
        const match = sorted.find((item) =>
            item.matchPaths.some((matchPath) => pathname === matchPath || pathname.startsWith(matchPath + "/"))
        );
        return match ? match.path : "/receptionist";
    };
        ...
        <aside className="w-64 bg-white border-r border-slate-100 min-h-screen flex flex-col flex-shrink-0">
            ...
            <div className="px-6 py-4 border-t border-slate-100">
                <button
                    ...
                    className="w-full flex text-white items-center gap-3 px-3 py-3 rounded-xl bg-red-500 text-sm font-semibold hover:bg-red-600 transition"
                >
